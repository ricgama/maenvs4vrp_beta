...
# Split Delivery Vehicle Routing Problem with Time Windows (SDVRPTW) environment
...
class Environment(AECEnv):
    ...
    * v0: Initial versions release (0.1.0)
    ...
    def step(self, td: TensorDict) -> TensorDict:
        ...
        Returns observation, reward, termination, truncation, info dicts for the agent, to mantain PettingZoo API compatibility.

        Parameters
        ----------
        Args:
            action (int): action to execute.
            update_obs (bool): if True the next return observactions are updated, else the return observations are the ones before the step is taken
         Returns
            observation (dict): observations dict.
            reward (dict): reward dict. Defaults to None
            termination (dict): termination state
            truncation (dict): truncation state. Defaults to None
            info (dict): environment information dict.
