...
class InstanceGenerator(InstanceBuilder):
    ...
    def augment_generate_instance(self, num_agents:int=20, 
        ...
        instance['depot_idx'] = self.depot_idx * torch.ones((*self.s_batch_size, 1), dtype = torch.int64, device=self.device).repeat(n_augment, 1)
        ...
        demands = torch.randint(low = 1, high=11, size = (*self.s_batch_size, num_nodes), dtype = torch.float, device=self.device).repeat(n_augment, 1)
        demands[:, self.depot_idx] = 0.0
        ...
        instance['end_time'] = time_windows[:, :, 1].gather(1, torch.zeros((*self.batch_size, 1), 
                                                                        dtype=torch.int64, device=self.device)).squeeze(-1)
        instance['capacity'] = self.capacity * torch.ones((*self.s_batch_size, 1), dtype = torch.float, device=self.device).repeat(n_augment, 1)
